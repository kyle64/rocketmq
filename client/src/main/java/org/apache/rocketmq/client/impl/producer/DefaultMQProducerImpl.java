...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            ...
            case CREATE_JUST:
                ...
                // 将当前producer实例注册到producerTable的map中
    ...
    /**
     * @Description: DefaultMQProducerImpl中最后调用的send方法
     *
     * @param msg 需要发送的消息message对象
     * @param communicationMode 消息发送模式
     * @param sendCallback 消息发送回调，只有ASYNC模式才有
     * @param timeout 超时时间
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            MessageQueue mq = null; // 要发送消息的队列
            Exception exception = null;
            SendResult sendResult = null;
            // 自动重试次数
            // 同步模式下，设置为retryTimesWhenSendFailed + 1，retryTimesWhenSendFailed默认为2，也就是总共重试3次
            // 异步/OneWay模式下，重试次数为1
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // 调用sendKernelImpl发送消息，核心方法
                        sendResult = this.sendKernelImpl(msg, mq, communicationMode, sendCallback, topicPublishInfo, timeout - costTime);
                        endTimestamp = System.currentTimeMillis();
                        // 如果开启了延迟容错机制，则更新延迟时间、不可用时间等
            ...
            // 重试完成后，如果有消息（但没有正常发送），返回sendResult
            if (sendResult != null) {
                return sendResult;
            }
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 从topicPublishInfoTable的map中查找topic
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        // 没有已经缓存的topicPublishInfo或者topicPublishInfo中的messageQueueList消息队列列表为空的话，则需要更新
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            // 从name server更新topic路由信息
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (null == brokerAddr) {
            // 获取不到broker地址则尝试从name server重新拉取topic信息再获取broker地址
        ...
        if (brokerAddr != null) {
            // 1。1 校验是否需要对broker地址的ip和port进行修改
            ...
            try {
                ...
                // 批处理消息无需再次创建unique id
                if (!(msg instanceof MessageBatch)) {
                    // 2。为message创建唯一id
                    MessageClientIDSetter.setUniqID(msg);
                }

                // 3。获取客户端配置中的namespace作为message的instanceId
                ...
                // 5。校验事物消息是否处于PREPARED
                final String tranMsg = msg.getProperty(MessageConst.PROPERTY_TRANSACTION_PREPARED);
                if (tranMsg != null && Boolean.parseBoolean(tranMsg)) {
                    sysFlag |= MessageSysFlag.TRANSACTION_PREPARED_TYPE;
                }

                // 6。检查是否有扩展的禁用hook，有就执行
                ...
                // 7。执行发送消息之前钩子方法(executeSendMessageHookBefore)
                ...
                switch (communicationMode) {
                    case ASYNC:
                        ...
                        // 如果消息被压缩过，需要将 msg body 置为压缩之前的 body，使用 clone 的消息进行消息发送，目的是解决消息重试发送时会被重复压缩的问题
                ...
                // 10。执行发送消息之后的钩子方法(executeSendMessageHookAfter)
                if (this.hasSendMessageHook()) {
